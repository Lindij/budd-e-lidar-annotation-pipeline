...
- [x] Validate that all new rosbags follow `_YYYY-MM-DD-HH-MM-SS.bag` naming.
- [x] Confirm `data/interim/<bag_id>_pcd` and `data/processed/<bag_id>/<model>` naming for every bag.

## Configs and Scripts
- [x] Clean `configs/upload_tracks/` and move legacy configs to `configs/upload_tracks/legacy/`.
- [x] Remove or populate empty `configs/pcdet/` if unused.
- [x] Keep a single default upload config for the current pipeline.
